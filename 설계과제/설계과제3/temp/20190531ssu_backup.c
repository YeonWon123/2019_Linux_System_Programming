...
void backup_function(backup_list *data) {
	...
	// 이거를 만들어서 써먹어 보자
	// 근데 링크드리스트가 더 나을거 같기도.... (삭제와 삽입이 빈번하게 일어남)
	char n_name_save[1024][1024];
	...
	// 탐방시 . ..는 제외하는데 기말고사 문제 19번을 볼것
	// 오름차순 정렬의 경우 recover에서 사용한다.
	// 이때 옵션처리도 해 주어야 하는데 이는 temp.opt_data랑 같게 하면 된다.
	

	// deep copy를 시도하는 함수를 만들어 주면 좋을거 같다. (이제 최적화하면 됨)
	...
	while (1) {
		...
		// n 옵션이 있는 경우
		// n_name_save와 n_count 이용
		if (temp.opt_data.n_flag == 1) {
			// 만약 백업한 파일의 개수가 NUMBER개보다 작다면, n_name_save에 그 이름 등록
			if (n_count < temp.opt_data.n_number) {
				strcpy(n_name_save[n_count], name_second);
				n_count++;
			}
			else {
				...
				sprintf(name_first, "%s/%s_", backup_dir, temp.p_filename);
				strcat(command, name_first);
				strcat(command, n_name_save[n_index]);
				printf("%s\n", command);
				system(command);

				// 최근 백업한 파일은 n_name_save[n_index]에 저장한다.
				strcpy(n_name_save[n_index], name_second);
				n_index++;

...
void rpath_to_fpath(char *r_path, char *f_path) {
	if (r_path[0] != '/') {
		...
		if (r_path[0] == '.' && r_path[1] == '/') {
			for (int i = 0; ; i++) {
				newpath[i] = r_path[i+2];
				if (newpath[i] == '\0')
					break;
			}
			strcat(f_path, newpath);
		}
		else {
...
void add(char *token[10], int argc)
{
	// <filename>과 [PERIOD]가 있는지 확인, 없으면 에러 처리
	if (argc < 3) {
	...
	// 절대경로의 길이가 255byte를 넘을 시 에러 처리
	if (strlen(filename) > 255) {
		fprintf(stderr, "%s's fullpath : %s\n", token[1], filename);
		fprintf(stderr, "fullpath's length is over at 255byte!\n");
		return;
	}
...
void recover(char *token[10], int argc)
{
	...
	// <FILENAME>이 존재하는지 확인, 존재하지 않는다면 에러
	if (access(token[1], F_OK) != 0) {
		fprintf(stderr, "%s doesn't exist!\n", token[1]);
		return;
	}
	...
	rpath_to_fpath(token[1], filename);
	if (search_list(filename) != 0) {
		fprintf(stderr, "%s doesn't exist in backup_list!\n", filename);
		return;
	}
	else {
	...
	k++;
	int i_k;
	for (int i_k = k; i_k <= strlen(filename)-1; i_k++) {
		p_filename[i_k-k] = filename[i_k];
	}
	p_filename[i_k-k] = '\0';
	...
	// 일단 디렉토리를 탐색해야 할듯
	struct dirent **namelist;
	char backup_file_list[1024][255];
	int list_count = 0;
	int cont_cnt = scandir(backup_dir, &namelist, NULL, alphasort);

	for (int i = 0; i < cont_cnt; i++) {
#ifdef DEBUG
		printf("%d. namelist[%d]->d_name : %s\n", i+1, i, namelist[i]->d_name);
		printf("p_filename : %s\n", p_filename);
#endif
		if (strstr(namelist[i]->d_name, p_filename) == NULL)
			continue;
		else {
			strcpy(backup_file_list[list_count], namelist[i]->d_name);
			list_count++;
		}
	}
	...
	// 백업 파일이 존재하지 않는 경우
	if (list_count == 0) {
		fprintf(stderr, "%s's backup_file doesn't exist!\n", token[1]);
		return;
	}
	else {
		// 존재할 경우 먼저 표준 출력으로 사용자에게 보여줌
		printf("0. exit\n");

		for (int i = 1; i <= list_count; i++) {
			struct stat statbuf;
			char f_filename[255];
			sprintf(f_filename, "%s/%s", backup_dir, backup_file_list[i-1]);
			if (lstat(f_filename, &statbuf) < 0) {
				fprintf(stderr, "lstat error for %s\n", f_filename);
#ifdef DEBUG
				exit(0);
#endif
			...
			char f_time[13];
			int lengths = strlen(backup_file_list[i-1]) - 1;
			for (int j = 0; j <= 11; j++) {
				f_time[j] = backup_file_list[i-1][lengths - 11 + j];
			}
			f_time[12] = '\0';

			printf("%d. %s	%ldbytes\n", i, f_time, statbuf.st_size);
		}
		...
		scanf("%d", &choose);
		getchar();
		if (choose > list_count || choose < 0) {
			fprintf(stderr, "wrong choose!\n");
			return;
		}
		else if (choose == 0) {
			return;
			...
			char f_filename[255];
			sprintf(f_filename, "%s/%s", backup_dir, backup_file_list[choose-1]);
			// backup_file_list[i-1]에 있는 파일로 백업한다는 이야기임
			// n옵션이 있을 경우는 그 파일 이름으로 백업하면 되고,
			// n옵션이 없을 경우는 덮어쓰면 됨
			// char filename[1024] : 원본 파일
			// char f_filename[255] : 백업할 파일 대상
			// char n_filename[255] : 새로 백업할 파일 이름, n옵션이 있을 시 작동
			char command[1024];
			if (n_flag == 1) {
				// n옵션 존재
				// cp [백업할 파일] [붙여넣을 파일 이름]
				strcpy(command, "cp ");
				strcat(command, f_filename);
				strcat(command, " ");
				strcat(command, n_filename);
#ifdef DEBUG
				printf("command : %s\n", command);
#endif
				system(command);	
...
#endif
				system(command);
				// 원본 파일이 변경되었으므로, 변경된 파일의 내용을 출력해 준다.
				printf("[변경된 파일의 내용]\n");
				strcpy(command, "cat ");
				strcat(command, filename);
				system(command);

				...
				if ((strcmp(buf_token2[i_token], "||") == 0) || (strcmp(buf_token2[i_token], "&&") == 0) ||
					...
					for (i_token_2 = 0; i_token_2 < token_count_student; i_token_2++) {
						...
						if ((strcmp(buf_token[i_token_2], "||") == 0) || (strcmp(buf_token[i_token_2], "&&") == 0) ||
							...
							if (correct_flag == 1) {
								printf("성공했으니 더 비교할 필요가 없겠죠?\n");
								break;
							}

						}

						printf("학생 파일 각각에 대해 비교를 종료하였습니다!\n");
					}
			...
			if ((strcmp(buf_token2[i_token], "<") == 0) || (strcmp(buf_token2[i_token], ">") == 0)) {
				...
				for (i_token_2 = 0; i_token_2 < token_count_student; i_token_2++) {
					...
					if ((strcmp(buf_token[i_token_2], "==") == 0) || (strcmp(buf_token[i_token_2], "!=") == 0)) {
						...
						printf("밑의 줄은 무시하세요! 정답 토큰의 왼쪽과 학생 토큰의 오른쪽이 같고\n");
						printf("학생 토큰의 왼쪽과 정답 토큰의 오른쪽이 같지만, 성립하지 못했습니다.\n");
					}

					printf("이런! 정답 토큰의 왼쪽과 학생 토큰의 오른쪽이 다르네요!\n");
					printf("학생 파일 각각에 대해 비교를 종료하였습니다!\n");
				}

...
// 변환 후 줄이 변환 전 어느 줄에 있었는지를 나타내는 배열
int after_arr[1024];
...
#ifdef DEBUG
			printf("r_flie_name : %s\n", r_file_name);
			printf("str_buffer_tran : %s\n", str_buffer_tran);
#endif
...
int main(int argc, char *argv[]) {
	...
	int main_sw = 0;	// main.c
	int main_main_sw = 0;	// int main
...
//	exit(0);
	...
	// 괄호의 개수( {개수 - }개수)를 세는 변수 도입
	int left_side_count = 0;
	int right_side_count = 0;
	for (i = 0; i < line_count; i++) {

		if (i == line_count-1 && r_flag == 1) {
			r_opt_func(0, buffer[i], NULL);
			break;
		}
		...
		// 괄호의 개수 확인
		if (strstr(buffer[i], "{") != NULL) left_side_count++;
		else if (strstr(buffer[i], "}") != NULL) right_side_count++;
		...
		if (strstr(buffer[i], buf_temp) != NULL) {
			...
			strcat(transfer_buf[transfer_line], "\n");
			after_arr[transfer_line] = i;
#ifdef r_option
			// r옵션이 있는 경우
			// printf("r_option 출력!\n");
			// printf("printf문구\n");
			// printf("buffer[i] : %s\n", buffer[i]);
			// printf("transfer_buf[transfer_line] : %s\n", transfer_buf[transfer_line]);

			if (r_flag == 1)
				r_opt_func(i, buffer[i], transfer_buf[transfer_line]);
#endif
	...
	//		}
		...
		// 2019-04-12 완성
		// (1) public class가 있는지 살피기 (있으면 public class 다음에 나오는 변수명.c 파일을 생성) - main_sw(switch) = 1; 로 만든다.
		...
		if (strstr(buffer[i], buf_temp) != NULL) {
			...
			// r옵션이 있는 경우
			//printf("r_option 출력!\n");
			//printf("public class문구\n");
			//printf("buffer[i] : %s\n", buffer[i]);
			//printf("transfer_buf[transfer_line] : 없음!\n");


				// r옵션이 있으면 뭘 바꾸고 있는지 기록
				strcpy(r_file_name, n_buf_temp);
				r_opt_func(-1, NULL, NULL);
				r_opt_func(0, buffer[i], NULL);
			}
#endif
...
//			exit(0);
		...
		if (strstr(buffer[i], buf_temp) != NULL) {
			...
			// r옵션이 있는 경우
			//	printf("r_option 출력!\n");
			//	printf("public static final문구\n");
			//	printf("buffer[i] : %s\n", buffer[i]);
			//	printf("transfer_buf[transfer_line] : %s\n", transfer_buf[transfer_line]);
				r_opt_func(0, buffer[i], transfer_buf[transfer_line]);
			}
#endif
...
//			exit(0);
		...
		// (1) public static void main이 있는지 살피기 (있으면 int main(int argc, char *argv[])를 생성) - main_main_sw = 1;로 만든다.
		...
		if (strstr(buffer[i], buf_temp) != NULL) {
			...
			// r옵션이 있는 경우
			//printf("r_option 출력!\n");
			//printf("public static void main문구\n");
			//printf("buffer[i] : %s\n", buffer[i]);
			//printf("transfer_buf[transfer_line] : %s\n", transfer_buf[transfer_line]);
...
//					exit(0);
		...
		for (j = 0; j < SIZE; j++) {
			...
			//printf("transfer_buf[transfer_line] : %s\n", transfer_buf[transfer_line]);
					...
					// true이면 a+로, false이면 w+로
					if (strcmp(buffer_token[4], "true") == 0)
						strcat(transfer_buf[transfer_line], ", \"a+\");\n");
					else
					...
					if (exception_sw == 1) {
						...
						char str_buffer_tran[1024][1024];
						int tran_size = 0;
...
#ifdef r_option
						if (r_flag == 1) {
							strcpy(str_buffer_tran[tran_size], transfer_buf[transfer_line]);
							tran_size++;
						}
#endif
						after_arr[transfer_line] = i;
						...
						if (r_flag == 1)
							r_opt_func_ss(i, buffer[i], tran_size, str_buffer_tran);
#endif
		...
		printf("나머지는 그대로 투입!\n");
#endif
		if (main_main_sw == 0 && strstr(buffer[i], "{") != 0) continue;

		// 맨 마지막 tab키는 무시한다.
		if (left_side_count - right_side_count == 0) {
			strcpy(transfer_buf[transfer_line], "\n");
		}
		else {
			...
			if (strstr(transfer_buf[transfer_line], "null") != 0) {
				// null의 경우 소문자를 대문자로 바꾸어야 한다.
				int index_tra = 0;
				while(((transfer_buf[transfer_line][index_tra] != 'n') || (transfer_buf[transfer_line][index_tra+1] != 'u') || (transfer_buf[transfer_line][index_tra+2] != 'l') || (transfer_buf[transfer_line][index_tra+3] != 'l'))) {
					index_tra++;
				}
				transfer_buf[transfer_line][index_tra] = 'N';
				transfer_buf[transfer_line][index_tra+1] = 'U';
				transfer_buf[transfer_line][index_tra+2] = 'L';
				transfer_buf[transfer_line][index_tra+3] = 'L';
			}
		}
			...
			//printf("r_option 출력!\n");
			//printf("나머지 그냥 들어가는 문구\n");
			//printf("buffer[i] : %s\n", buffer[i]);
			//printf("transfer_buf[transfer_line] : %s\n", transfer_buf[transfer_line]);

		if (r_flag == 1) {
			// 괄호의 개수 차이 ({의 개수 - }의 개수)가 0보다 작거나 같을 경우 변환하지 않음을 표시
			if (left_side_count - right_side_count > 0)
				r_opt_func(0, buffer[i], transfer_buf[transfer_line]);
			else
				r_opt_func(0, buffer[i], NULL);
...
//	exit(0);
	...
	if (class_count == 0) {
		...
		char str_save[512];
		head_count_arr_count = 0;
		head_count_arr_index = 0;
				...
				if (strstr(transfer_buf[k], function_str_part) != NULL) {
					...
					if (head_sw == 0) {
						...
						while((function_str_part = strtok(NULL, "#")) != NULL) {
							header_count++;	header_count_part++;
							strcpy(header_store[header_count], "#");
							strcat(header_store[header_count], function_str_part);
						}
			...
			for (k = 0; k < class_count && done == 0; k++) {
				...
				if (strstr(transfer_buf[k], function_str_part) != NULL) {
					...
					if (head_sw == 0) {
						...
						header_count++;	header_count_c++; header_count_part++;
	...
	//			printf("검사결과 : %s\n", strstr(function_str_part, transfer_buf[k]));
				...
				if (strstr(transfer_buf[k], function_str_part) != NULL) {
					...
					if (head_sw == 0) {
						...
						while((function_str_part = strtok(NULL, "#")) != NULL) {
							header_count++; header_count_part++; header_count_main++;
							strcpy(header_store[header_count], "#");
							strcat(header_store[header_count], function_str_part);
	...
	// r 옵션의 경우 위의 과정을 실행하면서 할 수는 없을까? 고민해보자! -> 해결!
	...
	if (l_flag == 1) {
		...
		if (class_count != 0) {
			...
			if (r_flag == 0)
				printf("%s line number is %d lines\n", fd_file_tf, header_count + transfer_line - 1);
			else
